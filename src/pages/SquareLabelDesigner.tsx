          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-lg font-semibold flex items-center gap-2">
                ...
                Background colour for big squares
              </Label>
              <div className="grid grid-cols-5 gap-3">
                {backgrounds.map((bg) => (
                  <button
                    key={bg.name}
                    onClick={() => updateLabel('backgroundColorOdd', bg.value)}
                    className={`w-12 h-12 rounded-lg border-2 transition-all hover:scale-110 ${
                      labelData.backgroundColorOdd === bg.value
                        ? 'border-primary scale-110 ring-2 ring-primary ring-offset-2'
                        : 'border-border hover:border-primary/50'
                    } ${bg.bg}`}
            ...
            <div className="space-y-2">
              <Label className="text-lg font-semibold flex items-center gap-2">
                <Wand2 className="w-5 h-5 text-primary" />
                Background colour for small squares
              </Label>
              <div className="grid grid-cols-5 gap-3">
                {backgrounds.map((bg) => (
                  <button
                    key={`even-${bg.name}`}
                    onClick={() => updateLabel('backgroundColorEven', bg.value)}
                    className={`w-12 h-12 rounded-lg border-2 transition-all hover:scale-110 ${
                      labelData.backgroundColorEven === bg.value
                        ? 'border-primary scale-110 ring-2 ring-primary ring-offset-2'
                        : 'border-border hover:border-primary/50'
                    } ${bg.bg}`}
                    style={{ backgroundColor: bg.value }}
